...
/**
 * Extension of Spring LDAP's LdapTemplate class which adds extra functionality
 ...
 * @since 2.0
 */
public class CustomSpringSecurityLdapTemplate extends LdapTemplate {
	// ~ Static fields/initializers
	// =====================================================================================
	// private static final Log logger =
	// LogFactory.getLog(CustomSpringSecurityLdapTemplate.class);
	...
	public CustomSpringSecurityLdapTemplate(ContextSource contextSource) {
	 ...
	 *            the named attributes which will be retrieved from the directory
	 *            entry.
	 *
	...
	/**
	 * Performs a search using the supplied filter and returns the union of the
	 * values of the named attribute found in all entries matched by the search.
	 * Note that one directory entry may have several values for the attribute.
	 * Intended for role searches and similar scenarios.
	 *
	 * @param base
	 *            the DN to search in
	 * @param filter
	 ...
	 * @param params
	 *            the parameters to substitute in the search filter
	 * @param attributeName
	 *            the attribute who's values are to be retrieved.
	 *
	 * @return the set of String values for the attribute as a union of the values
	 *         found in all the matching entries.
	 */
	 ...
	 * directory entry, and uses the supplied mapper to create the object from that
	 * entry.
	 * <p>
	 * Ignores <tt>PartialResultException</tt> if thrown, for compatibility with
	 * Active Directory (see
	 * {@link LdapTemplate#setIgnorePartialResultException(boolean)}).
	 *
	 * @param base
	 *            the search base, relative to the base context supplied by the
	 *            context source.
	 * @param filter
	...
	public static DirContextOperations searchForSingleEntryInternal(DirContext ctx, SearchControls searchControls,
		...
		// ctx.search(toDC("DISDIVSA"), filter, searchControls);
		/*
		 * if (logger.isDebugEnabled()) { logger.debug("Searching for entry under DN '"
		 * + ctxBaseDn + "', base = '" + searchBaseDn + "', filter = '" + filter + "'");
		 * }
		 */
		...
		try {
			...
			while (resultsEnum.hasMore()) {
				...
				// System.out.println(searchResult.getAttributes().get("CN"));
				// System.out.println(searchResult.getNameInNamespace());
				DirContextAdapter dca = new DirContextAdapter(searchResult.getAttributes(),
						new DistinguishedName(searchResult.getName()));
				// dca.
	...
	/**
	 * We need to make sure the search controls has the return object flag set to
	 * true, in order for the search to return DirContextAdapter instances.
	 * 
	...
	/**
	 * Sets the search controls which will be used for search operations by the
	 * template.
	 *
